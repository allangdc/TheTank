...
QSize TMXFiles::MapSize()
{
    int w, h;
    w =  MatrixSize().width() * TiledSize().width();
    h =  MatrixSize().height() * TiledSize().height();
    return QSize(w,h);
}
...
QPixmap TMXFiles::TileImageByID(int layer, int id)
{
    int max_x = qFloor((qreal) map->tileset.image.width / (qreal) map->tilewidth);
    int max_y = qFloor((qreal) map->tileset.image.height / (qreal) map->tileheight);
    int y = id / max_x;
    int x = id - y*max_x;
    return TileImage(layer, x, y);
}
